...
from diffspin.load_bpl_histories import load_histories, TASSO, BEBOP
from diffspin.fit_spin_helpers import fit_lgspin
from diffspin.fit_spin_helpers import get_outline, get_outline_bad_fit, get_header
import subprocess
import h5py

TMP_OUTPAT = "_tmp_spin_fits_rank_{0}.dat"
...
if __name__ == "__main__":
    ...
    if args.indir == "TASSO":
        ...
        indir = args.indir
    halo_ids, spins, log_mahs, t, LGM_MIN = load_histories(indir, "spin")
    ...
    with open(rank_outname, "w") as fout:
        ...
        for i in range(nhalos_for_rank):
            ...
            spinh = spins_for_rank[i, :]

            p_best, loss, method, loss_data = fit_lgspin(t, spinh, lgmah, LGM_MIN)
